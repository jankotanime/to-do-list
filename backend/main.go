...
func getTasksFromEvent() ([]EventTask, error) {
	err := godotenv.Load()
	if err != nil {
		log.Fatal("Błąd przy ładowaniu pliku .env")
	}
	...
	// Zapytanie do bazy danych
	rows, err := dbpool.Query(context.Background(), "SELECT t.id_event_task, t.plot, t.deadline, t.done, t.id_event FROM event_tasks t INNER JOIN events e ON t.id_event = e.id_event WHERE e.checked = true")
	if err != nil {
		return nil, fmt.Errorf("błąd pobierania danych: %v", err)
	}
	defer rows.Close()
...
func CheckEventTaskToDB(id int, done bool) error {
	err := godotenv.Load()
	if err != nil {
		log.Fatal("Błąd przy ładowaniu pliku .env")
	}
	...
	// Zapytanie do wstawienia nowego zadania
	_, err = dbpool.Exec(context.Background(), "UPDATE event_tasks SET done=$2 WHERE id_event_task=$1", id, done)
	if err != nil {
		return fmt.Errorf("błąd podczas dodawania zadania: %v", err)
	}
...
func eventTaskHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		err = CheckEventTaskToDB(requestData.ID, requestData.Done)
		if err != nil {
			http.Error(w, "Błąd podczas dodawania zadania do bazy", http.StatusInternalServerError)
			return
		}
...
func eventTaskForEventsHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		events, err := getTasksFromEvent()
		if err == nil {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK) // Ustaw status 200 (OK)
			json.NewEncoder(w).Encode(events)
...
func main() {
	...
	http.HandleFunc("/api/event/tasks/specific", eventTaskForEventsHandler)
